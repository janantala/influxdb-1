...
func TestMetaService_DropDataNode(t *testing.T) {
	...
	n1, err := c.CreateDataNode("foo:8180", "bar:8281")
	if err != nil {
		t.Fatal(err)
	}

	n2, err := c.CreateDataNode("foo:8280", "bar:8381")
	if err != nil {
		t.Fatal(err)
	}
	...
	// Dropping the second data node will orphan the shard, and since
	// it's the shard group's only shard, the group will be marked as
	// deleted.
